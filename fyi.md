...
| `set_tts_base` | url |
| `set_tts_key` | key |
| `speech` | input, voice, file, backend, model, temperature, speed, exaggeration, cfg_weight, stability, similarity_boost, seed, response_format, instructions, device |
...
- **`port`**: Port to check (default from QWEN3_TTS_PORT env var or 7812)
- **`timeout`**: Timeout in seconds (default 2)
